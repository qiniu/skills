...
#
# 注意事项：
#   - 脚本在远程 VM 上以 root 身份执行，stdin 已重定向到 /dev/null
#   - 脚本必须全程非交互式（所有命令不能等待用户输入，否则挂死并超时失败）
#   - cloud-init 必须保留，不能删除或禁用
#   - 环境清理（apt clean、cloud-init clean 等）由 image-cli.py 统一处理，
#     此脚本只需关注软件安装和配置
#   - 如需手动制作镜像，清理步骤参考 references/image-building.md
...
#     防止 apt/dpkg 弹出 debconf 配置界面（如 tzdata 要求选择时区）
#   NEEDRESTART_MODE=a
#     Ubuntu 22.04+ 安装内核/libc 相关包后，needrestart 会询问"是否重启服务"
#     设为 'a'（automatic）自动重启，跳过交互提示
#   UCF_FORCE_CONFFOLD=1
#     apt 升级时若检测到配置文件冲突，自动保留旧版本，不弹出对话框
# ============================================================================
...
#
#   第三方安装脚本（curl | bash）:
#     先确认是否支持 --yes / --non-interactive / --skip-setup 等标志
#     示例：curl -fsSL https://example.com/install.sh | bash -s -- --yes
#     若不支持静默标志，需在执行前设置相应环境变量或预先创建应答文件
#
#   Python pip:
#     pip install --quiet --no-input <pkg>
#
#   npm:
#     npm install --yes / CI=true npm install
# ============================================================================
# TODO: 根据你的应用修改以下命令
